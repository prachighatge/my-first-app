...
function App() {
  ...
  const handleSearch = () => {
    if (!section) return;
    if (!['jokes', 'quotes'].includes(section) && !query.trim()) return;
    ...
    switch (section) {
      ...
      case 'jokes':
        ...
        break;
      case 'quotes':
        apiUrl = 'https://api.quotable.io/random?timestamp=' + new Date().getTime();
        break;
      default:
        setLoading(false);
        return;
      ...
      .then(data => {
        switch (section) {
          ...
          case 'jokes':
            ...
            break;
          case 'quotes':
            console.log("Quote API response:", data); // ✅ Debug log
    ...
    <div className="app-container">
      ...
      {(!section || results.length === 0) && (
        <div className="grid-container">
          ...
          <div className="card" onClick={() => handleCardClick('quotes')}>
            <span className="emoji">💬</span><p>Quotes</p>
          </div>
        </div>
      )}
      ...
      {section && (
        <div className="search-controls">
          ...
          <button onClick={handleSearch}>
            {['quotes'].includes(section) ? 'Get Quote' : 'Search'}
          </button>
        </div>
      )}
